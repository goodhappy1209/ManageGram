...
NSString * CustomTransitionControllerAssociationKey = @"CustomTransitionControllerAssociationKey";

#define AD_NAVIGATION_BAR_HEIGHT 44.0f
#define AD_TOOLBAR_HEIGHT 44.0f
#define AD_Z_DISTANCE 1000.0f

@interface CustomTransitionController (Private)
- (void)_initialize;
- (void)_setupLayers:(NSArray *)layers;
- (void)_teardownLayers:(NSArray *)layers;
- (void)_transitionfromView:(UIView *)viewOut toView:(UIView *)viewIn withTransition:(CustomTransition *)transition;
@end
...
- (void)loadView {
    ...
    _containerView = [[CustomTransitionView alloc] initWithFrame:CGRectMake(self.view.frame.origin.x, self.view.frame.origin.y + navigationBarHeight, self.view.frame.size.width, self.view.frame.size.height - navigationBarHeight - toolbarHeight)];
...
- (void)pushViewController:(UIViewController *)viewController withTransition:(CustomTransition *)transition {
    ...
    objc_setAssociatedObject(viewController, (__bridge const void *)(CustomTransitionControllerAssociationKey), self, OBJC_ASSOCIATION_ASSIGN);
    ...
    if (transition) {
        ...
        [_transitions addObject:[CustomTransition nullTransition]];
    }
    ...
    transition.type = CustomTransitionTypePush;
...
- (NSArray *)popToViewController:(UIViewController *)viewController withTransition:(CustomTransition *)transition {
    ...
    CustomTransition * lastTransition = [_transitions lastObject];
...
@implementation CustomTransitionController (Private)
...
- (void)_transitionfromView:(UIView *)viewOut toView:(UIView *)viewIn withTransition:(CustomTransition *)transition {
    ...
    } else if ([transition isKindOfClass:[CustomDualTransition class]]) { // CustomDualTransition
